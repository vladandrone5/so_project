...
-added calculate score for total hunt and for username
*/
...
void calc_score_by_user(char *hunt_id, char *user) {
    if(system("gcc -Wall -o c calculator.c") == -1) {
        perror("error compiling calc.c\n");
        exit(-1);
    }


    char op[1024];
    snprintf(op, sizeof(op), "./c %s %s", hunt_id, user);
    if(system(op) == -1) {
...
void process_operation(char *operation, char *hunt_id, char *treasure_id) {
    ...
    else if(strcmp(operation, "--calc_score") == 0) {
        calc_score_by_user(hunt_id, treasure_id);
        return;
    }
    else {
